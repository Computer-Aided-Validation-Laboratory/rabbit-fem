...
"""Example 1: Mesh and run the 2D elastic dogbone tensile simulation."""
...
def main() -> None:
    """Generate 2D dogbone mesh via Gmsh and run elastic simulation."""
    geo_path: Path = dogbone_geo_path(EDims.TWOD)
    input_path: Path = dogbone_input_path(EDims.TWOD, is_plastic=False)

    with tempfile.TemporaryDirectory() as tmp_dir:
        tmp_path = Path(tmp_dir)
        msh_path = tmp_path / "dogbone2d.msh"
