...
- **Follow-up finding from the first init-enabled round**: `git submodule update` then refused with `fatal: destination path '.../moose/petsc' already exists and is not an empty directory` — the cache restore creates `moose/petsc/` containing only `arch-windows-opt/`. Fix: relocate restored build outputs (`arch-windows-opt`, `installed`) to `*.​__rabbit_backup` siblings so the clone can proceed, move them back after success; orphaned backups from killed runs are resurrected (or pruned if superseded) first. State machine validated locally across hollow/complete/orphan/stale-duplicate scenarios (moves only when keep-present + sentinel-absent; complete trees untouched).
