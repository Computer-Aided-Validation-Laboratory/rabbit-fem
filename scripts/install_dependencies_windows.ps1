...
if ($Stage -in @("all", "rabbit")) {
    ...
    $preflight = "LIBMESH_CONFIG=$RepoRootPosix/moose/libmesh/installed/bin/libmesh-config && if [ ! -x `"`$LIBMESH_CONFIG`" ]; then echo 'ERROR: libmesh-config missing or not executable:'; ls -la $RepoRootPosix/moose/libmesh/installed/bin/; exit 1; fi && echo '--- libmesh-config --cxx ---' && METHOD=$Method `"`$LIBMESH_CONFIG`" --cxx && echo '--- libmesh-config --cppflags ---' && METHOD=$Method `"`$LIBMESH_CONFIG`" --cppflags && echo '--- libmesh-config --include ---' && LIBMESH_INCLUDE_OUT=`$(METHOD=$Method `"`$LIBMESH_CONFIG`" --include) && echo `"`$LIBMESH_INCLUDE_OUT`" && echo `"`$LIBMESH_INCLUDE_OUT`" | grep -q 'petsc' || (echo 'ERROR: libmesh-config --include has no PETSc paths; libmesh was likely configured without PETSc (check PETSC_DIR/PETSC_ARCH at libmesh configure time)'; exit 1) && echo '--- PETSc headers ---' && ls $RepoRootPosix/moose/petsc/arch-windows-opt/include/petscconf.h $RepoRootPosix/moose/petsc/include/petscsys.h"
