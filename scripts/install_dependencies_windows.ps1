...
if (-not (Test-Path $HitExe)) {
    ...
    $patchWasp = "cd moose/framework/contrib/wasp && sed -i 's/defined(__GNUC__)/defined(__GNUC__) \&\& defined(_TWO_DIGIT_EXPONENT)/g' waspcore/Format.h && sed -i 's/\$<BUILD_INTERFACE:\${CMAKE_CURRENT_SOURCE_DIR}>/\$<BUILD_INTERFACE:\${CMAKE_CURRENT_SOURCE_DIR}> \"\$<BUILD_INTERFACE:\${CMAKE_CURRENT_SOURCE_DIR}\/..>\"/g' waspcore/CMakeLists.txt"
